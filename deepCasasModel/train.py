    ...
    for dataset in data.datasetsNames:
        ...
        for train, test in kfold.split(X, Y):
            ...
            model.fit(X_train_input, Y[train], validation_split=0.2, epochs=epochs, batch_size=64, verbose=1,
                      callbacks=[csv_logger, model_checkpoint], sample_weight=sample_weight)
            ...
            print('Balanced Accuracy: {:.2f}'.format(balanced_accuracy_score(Y[test], classes)))
            print('Confusion matrix:')
            labels = list(dictActivities.values())
            # print(confusion_matrix(list(Y[test]), classes, labels))
            conf_matrix = confusion_matrix(list(Y[test]), classes)
            print(conf_matrix)

            # Display confusion matrix as a heatmap
            plt.figure(figsize=(10, 8))
            sns.heatmap(conf_matrix, annot=True, cmap="viridis", fmt='g', cbar=True, xticklabels=target_names, yticklabels=target_names)
